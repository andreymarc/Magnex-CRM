...
const translations = {
  en: {
    ...
    register: {
      ...
      companySubdomainHint: "Enter the company name in English, without spaces between letters",
      next: "Next",
      previous: "Previous",
      submit: "Open System",
...
export default function RegisterPage() {
  ...
  const validateStep2 = () => {
    ...
    if (!formData.companyName.trim()) {
      newErrors.companyName = language === 'en' ? 'Company name is required' : 'שם החברה נדרש'
    }
    
    if (!formData.companyPhone.trim()) {
      newErrors.companyPhone = language === 'en' ? 'Company phone is required' : 'טלפון בחברה נדרש'
    } else if (!/^[0-9+\-\s()]+$/.test(formData.companyPhone)) {
      newErrors.companyPhone = language === 'en' ? 'Invalid phone number' : 'מספר טלפון לא תקין'
    }
    
    if (!formData.companySubdomain.trim()) {
      newErrors.companySubdomain = language === 'en' ? 'Company subdomain is required' : 'שם החברה באנגלית נדרש'
    } else if (!/^[a-zA-Z0-9-]+$/.test(formData.companySubdomain)) {
      newErrors.companySubdomain = language === 'en' ? 'Only English letters, numbers, and hyphens allowed' : 'רק אותיות באנגלית, מספרים ומקפים מותרים'
    } else if (formData.companySubdomain.length < 3) {
      newErrors.companySubdomain = language === 'en' ? 'Subdomain must be at least 3 characters' : 'שם החברה חייב להכיל לפחות 3 תווים'
    }
  ...
  return (
    <div dir={isRTL ? 'rtl' : 'ltr'} className="min-h-screen bg-gradient-to-br from-green-50 via-white to-blue-50 antialiased relative overflow-hidden">
      ...
      <div className="relative z-10 flex items-center justify-center min-h-[calc(100vh-64px)] sm:min-h-[calc(100vh-80px)] py-4 sm:py-8 lg:py-12 px-3 sm:px-4 lg:px-8">
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl sm:shadow-2xl p-5 sm:p-8 lg:p-10 border border-gray-100">
            ...
            {planName && (
              <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-primary-50 rounded-lg sm:rounded-xl border border-primary-200">
                <p className="text-xs text-primary-600 font-semibold mb-0.5 sm:mb-1">{t.register.planSelected}</p>
                <p className="text-base sm:text-lg font-bold text-primary-700">{getPlanDisplayName()}</p>
              </div>
            )}
            ...
            <div className="text-center mb-6 sm:mb-8">
              ...
              <div className="flex items-center justify-center space-x-2 rtl:space-x-reverse mt-3">
                <div className={`h-2 w-8 sm:w-12 rounded-full transition-all ${currentStep >= 1 ? 'bg-primary-600' : 'bg-gray-300'}`}></div>
                <div className={`h-2 w-8 sm:w-12 rounded-full transition-all ${currentStep >= 2 ? 'bg-primary-600' : 'bg-gray-300'}`}></div>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
              ...
              <div>
                ...
                <div className="relative">
                  <FiUser className="absolute right-3 rtl:left-3 rtl:right-auto top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4 sm:w-5 sm:h-5" />
                  <input
                    type="text"
                    id="firstName"
                    name="firstName"
              ...
              <div>
                ...
                <div className="relative">
                  <FiMail className="absolute right-3 rtl:left-3 rtl:right-auto top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4 sm:w-5 sm:h-5" />
                  <input
                    type="email"
                    id="email"
                    name="email"
              ...
              <div>
                ...
                <div className="relative">
                  <FiPhone className="absolute right-3 rtl:left-3 rtl:right-auto top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4 sm:w-5 sm:h-5" />
                  <input
                    type="tel"
                    id="mobile"
                    name="mobile"
                ...
                {errors.mobile && (
                  <p className="mt-1 text-xs sm:text-sm text-red-600">{errors.mobile}</p>
                )}
              </div>
                </>
              ...
              {currentStep === 2 && (
                <>
                  {/* Company Name */}
                  <div>
                    <label htmlFor="companyName" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">
                      {t.register.companyName}
                    </label>
                    <div className="relative">
                      <FiHome className="absolute right-3 rtl:left-3 rtl:right-auto top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4 sm:w-5 sm:h-5" />
                      <input
                        type="text"
                        id="companyName"
                        name="companyName"
                  ...
                  {/* Company Phone */}
                  <div>
                    <label htmlFor="companyPhone" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">
                    ...
                    {errors.companyPhone && (
                      <p className="mt-1 text-xs sm:text-sm text-red-600">{errors.companyPhone}</p>
                    )}
                  </div>

                  {/* Company Subdomain */}
                  <div>
                    <label htmlFor="companySubdomain" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">
                      {t.register.companySubdomain}
                    </label>
                    <div className="relative">
                      <FiBriefcase className="absolute right-3 rtl:left-3 rtl:right-auto top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4 sm:w-5 sm:h-5" />
                      <input
                        type="text"
                        id="companySubdomain"
                    ...
                    {errors.companySubdomain && (
                      <p className="mt-1 text-xs sm:text-sm text-red-600">{errors.companySubdomain}</p>
                    )}
                    {formData.companySubdomain && !errors.companySubdomain && (
                      <p className="mt-1 text-xs text-gray-600">
                        {language === 'en' ? 'Your URL will be: ' : 'כתובת המערכת שלך תהיה: '}
                        <span className="font-mono text-primary-600">{formData.companySubdomain}.magnex-crm.com</span>
                      </p>
                    )}
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-2">
                ...
                {currentStep === 2 && (
                  <button
                    ...
                    className="w-full sm:w-auto sm:flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 py-3 sm:py-3.5 rounded-lg font-semibold text-base sm:text-lg transition-all duration-200 touch-manipulation"
                  >
                    {t.register.previous}
                  </button>
                )}
                ...
                <button
                  ...
                  className={`w-full ${currentStep === 2 ? 'sm:flex-1' : ''} bg-gradient-to-r from-green-500 to-emerald-600 text-white py-3 sm:py-3.5 rounded-lg font-semibold text-base sm:text-lg shadow-lg hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed touch-manipulation`}
                >
                  {isSubmitting ? (
                    <span className="flex items-center justify-center">
                      ...
                      <span className="text-sm sm:text-base">{language === 'en' ? 'Processing...' : 'מעבד...'}</span>
                    </span>
                  ) : (
                    currentStep === 1 ? t.register.next : t.register.submit
                  )}
                </button>
              </div>
